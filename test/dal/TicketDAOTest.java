...
//Test để lấy toàn bộ thông tin TicketDetail bị fail

    @Test
    public void testGetAllTicketDetails_Fail_EmptyList() throws SQLException {
        // Giả lập ResultSet không có bản ghi nào (next() = false)
        when(resultSet.next()).thenReturn(false);

        // Gọi DAO method
        List<TicketDetail> tickets = ticketDAO.getAllTicketDetails();

        // Kiểm tra kết quả trả về list rỗng
        assertNotNull(tickets); // Đảm bảo không null
        assertTrue(tickets.isEmpty()); // List phải rỗng

        // Kiểm tra có gọi executeQuery() không
        verify(preparedStatement).executeQuery();
    }
...
//Test để lấy ticketDetail dựa theo ID fail

    @Test
    public void testGetTicketDetailById_Fail_NotFound() throws SQLException {
        // Giả lập không có kết quả trả về (next = false)
        when(resultSet.next()).thenReturn(false);

        // Gọi DAO method
        TicketDetail ticket = ticketDAO.getTicketDetailById(999); // ID không tồn tại

        // Kiểm tra trả về null hoặc object rỗng (tuỳ cách bạn code)
        assertNull(ticket); // Nếu hàm return null khi không tìm thấy

        // Kiểm tra setInt đã được truyền ID đúng
        verify(preparedStatement).setInt(1, 999);

        // Kiểm tra executeQuery() đã được gọi
        verify(preparedStatement).executeQuery();
    }
...
//Test đổi trạng thái vé fail
    @Test
    public void testChangeTicketStatus_fail() throws SQLException {
        // Giả lập prepareStatement trả về mock
        when(connection.prepareStatement(anyString())).thenReturn(preparedStatement);
        // Giả lập executeUpdate thất bại (không có dòng nào được cập nhật)
        when(preparedStatement.executeUpdate()).thenReturn(0);

        // Gọi phương thức cần test
        boolean result = ticketDAO.changeTicketStatus(999); // ID không tồn tại

        // Kết quả mong muốn là false (do không có dòng nào bị update)
        assertFalse(result);

        // Kiểm tra setInt đúng
        verify(preparedStatement).setInt(1, 999);
        // Kiểm tra executeUpdate được gọi
        verify(preparedStatement).executeUpdate();
    }
...
//Test update ticketdetail fail
    @Test
    public void testUpdateTicketDetail_Fail() throws SQLException {
        // Giả lập executeUpdate trả về 0 (không có dòng nào bị cập nhật)
        when(preparedStatement.executeUpdate()).thenReturn(0);

        // Gọi hàm update
        ticketDAO.updateTicketDetail(1, 2, "0123456789", "Nguyen Van A", "Ha Noi");

        // Vẫn kiểm tra các lệnh set tham số
...
//Test insert ticketdetail thành công
    @Test
...
//Test insert ticketdetail fail

    @Test
    public void testInsertTicketDetail_Fail() throws SQLException {
        // Giả lập kết quả executeUpdate trả về 0 (insert thất bại)
        when(preparedStatement.executeUpdate()).thenReturn(0);
        ...
        assertFalse(result); // Vì insert thất bại
